...
import "../../uniswap/IPancakeRouter01.sol";
...
contract superPancakeBase is ERC20,ImportOracle,ReentrancyGuard {
    ...
    address public constant cakeRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    function getSwapRouterPathInfo(address token0,address token1)public view returns (address[] memory path){
        ...
        path = new address[](2);
        path[0] = token0 == address(0) ? WBNB : token0;
        path[1] = token1 == address(0) ? WBNB : token1;
    }
    ...
    function _setReward(uint256 index,uint8 _reward,bool _bClosed,address _rewardToken,uint256 _sellLimit) internal{
        if(index <rewardInfos.length){
            ...
            if(_rewardToken != address(0) 
                && IERC20(_rewardToken).allowance(address(this), cakeRouter) == 0){
                SafeERC20.safeApprove(IERC20(_rewardToken), cakeRouter, uint(-1));
            }
        }
